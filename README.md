...
Приложение состоит из четырех основных компонентов:
...
- **Config (config.py)** - централизованный модуль конфигурации, загружающий настройки из переменных окружения
...
Отправка изображения на обработку. Выполняется проверка расширения файла и его сигнатуры (magic bytes).

**Request:**
- Content-Type: `multipart/form-data`
- Body: `file` - изображение в формате PNG, JPG, BMP или TIFF
...
**Response (Ошибка валидации 400):**
```json
{
  "error": "Неподдерживаемое расширение файла. Допустимы: JPG, PNG, BMP, TIFF"
}
```
или 
```json
{
  "error": "Файл не является изображением (неверная сигнатура) или поврежден"
}
```
...
```json
{
  ...
  "file_url": "/processed/{task_id}.png"
}
```
